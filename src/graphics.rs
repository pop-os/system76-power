...
const INITRAMFS_HOOK_PATH: &str = "/etc/initramfs-tools/hooks/system76-power";

static INITRAMFS_HOOK_BOILERPLATE: &[u8] = br#"#!/bin/sh
...
static INITRAMFS_HOOK_NVIDIA: &[u8] = br#"# add Nvidia kernel modules to initramfs to enable external monitor during boot

manual_add_modules nvidia
manual_add_modules nvidia_drm
manual_add_modules nvidia_modeset
"#;

static INITRAMFS_HOOK_HYBRID: &[u8] = br#"# add Nvidia kernel modules to initramfs to enable external monitor during boot
...
static INITRAMFS_HOOK_COMPUTE: &[u8] = br#"# no modules added to initramfs
"#;

static INITRAMFS_HOOK_INTEGRATED: &[u8] = br#"# no modules added to initramfs
"#;
...
pub enum GraphicsDeviceError {
    ...
    #[error("failed to open system76-power initramfs hook file: {}", _0)]
    HookFileOpen(io::Error),
    #[error("failed to write to system76-power initramfs hook file: {}", _0)]
    HookFileWrite(io::Error),
...
impl Graphics {
    ...
    pub fn set_vendor(&self, vendor: &str) -> Result<(), GraphicsDeviceError> {
        ...
        {
            log::info!("Creating {}", INITRAMFS_HOOK_PATH);

            let mut hook_file = fs::OpenOptions::new()
                .create(true)
                .truncate(true)
                .write(true)
                .mode(0o744) // set INITRAMFS_HOOK_PATH to executable by root since it's a script
                .open(INITRAMFS_HOOK_PATH)
                .map_err(GraphicsDeviceError::HookFileOpen)?;

            let boilerplate = INITRAMFS_HOOK_BOILERPLATE;
            hook_file.write_all(boilerplate)
                .and_then(|_| hook_file.sync_all())
                .map_err(GraphicsDeviceError::HookFileWrite)?;

            let text = if vendor == "hybrid" {
                INITRAMFS_HOOK_HYBRID
            } else if vendor == "compute" {
                INITRAMFS_HOOK_COMPUTE
            } else if vendor == "nvidia" {
                INITRAMFS_HOOK_NVIDIA
            } else {
                INITRAMFS_HOOK_INTEGRATED
            };

            hook_file.write_all(text)
                .and_then(|_| hook_file.sync_all())
                .map_err(GraphicsDeviceError::HookFileWrite)?;            
        }
